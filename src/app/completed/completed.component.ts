...
todaysdate(date1: number): string {
...
document_front(_a:any){
  var document=[]
  var front: any[]=[]
    for (let index = 0; index < _a.length; index++) {
      document.push(_a[index].name);
      if( document.filter(element1 => element1.includes("Front_"))[0]){
        front.push(_a[index].downloadUrl);
      } else{
        front.push("https://previews.123rf.com/images/urfandadashov/urfandadashov1806/urfandadashov180601827/150417827-photo-not-available-vector-icon-isolated-on-transparent-background-photo-not-available-logo-concept.jpg");
      }
    }
...
document_back(_a:any){
  var document=[]

  var back: any[]=[]
    for (let index = 0; index < _a.length; index++) {
      document.push(_a[index].name);
      if( document.filter(element1 => element1.includes("Back_"))[0]){
        back.push(_a[index].downloadUrl);
      } else{
        back.push("https://previews.123rf.com/images/urfandadashov/urfandadashov1806/urfandadashov180601827/150417827-photo-not-available-vector-icon-isolated-on-transparent-background-photo-not-available-logo-concept.jpg");
      }
    }
...
issue_Escalations(_a:any){
  var issuename=[]
  if(_a!=null){
    for(let i=0;i<_a.length;i++){
      if(_a[i]['issueType']=='Escalations'){
        issuename.push(_a[i]['issueType'])
      }
    }
...
issue_Tracking_Update(_a:any){
  var issuename=[]
  if(_a!=null){
    for(let i=0;i<_a.length;i++){
      if(_a[i]['issueType']=='Tracking Update'){
        issuename.push(_a[i]['issueType'])
      }
    }
    return issuename.length
...
consigner(_a:any){
  if(_a.includes("- SIEMENS HEALTHCARE PVT LTD")){
    var abc =_a.split("- ")[1]
    return abc
  }else{
    return "--"
